    ...
    <div className="page">
      ...
      >
        <p className="waitlist-note">
          ⓘ Payments and accounts are not live yet. We'll email you when we launch.
        ...
        {submitted ? (
          <div className="waitlist-success">
            ...
            <p style={{ margin: 0 }}>
              We've saved your spot for <strong>{submitted.planInterest}</strong>.
              We'll reach out when ContributorOps launches.
            </p>
            <p style={{ margin: 0, fontSize: "0.82rem", color: "var(--muted)" }}>
              Target role: {submitted.targetRole} · Joined {new Date(submitted.submittedAt).toLocaleDateString()}
            </p>
          </div>
        ) : (
          <form className="waitlist-form" onSubmit={handleSubmit} noValidate>
            {errors.length > 0 && (
              <div className="waitlist-errors" role="alert">
                {errors.map((err) => (
                  <p key={err} style={{ margin: "0 0 4px", color: "var(--danger)", fontSize: "0.85rem" }}>
                    • {err}
                  </p>
                ))}
              </div>
